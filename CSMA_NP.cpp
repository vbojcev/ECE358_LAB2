    ...
    for (int m = 0; m < 3; m++) { //Loop through range of arrival rates
      ...
      while (LAN[transmitter].next() < T) {
        ...
        if (collisions.empty()) {
          for (int i = 0; i < LAN.size(); i++) {
            int distance = abs(transmitter - i);
            double busyTime = LAN[transmitter].next() + propTimeIndex[distance] + transTime;
            if (busyTime < T && LAN[i].next() <= busyTime) {
              int npCollisions = 0;
              while (LAN[i].next() < T && LAN[i].next() <= busyTime) {
                LAN[i].npBackOff();
                if (LAN[i].next() <= busyTime) {
                  npCollisions++;
                }
                if (npCollisions == 10) {
                  LAN[i].send();

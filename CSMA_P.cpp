...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 100; i++) {  //To save time and not do astronomical amounts of double-precision floating point computations
    ...
    for (int m = 0; m < 3; m++) { //Loop through range of arrival rates
      ...
      while ((next = LAN[transmitter].next()) < T) {
        ...
        if (!maxDist) {
          for (int i = 0; i < LAN.size(); i++) {
            int distance = abs(transmitter - i);
            if (LAN[i].next() <= next + propTimeIndex[distance] + transTime) {
              LAN[i].wait(next + propTimeIndex[distance] + transTime); //Nodes (including the sender!) must wait until the entire packet has passed.
            }
          }
